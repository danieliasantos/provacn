...
\end{question}
\begin{comment}
	Questão 1. Para $f(x) = 3x - 1$, definida no intervalo $x \in (1, +\infty)$, sejam $x\textsubscript{0} = 1,5, x\textsubscript{1} = 1,6$ e $x\textsubscript{2} = 1,9$. Construa polinômios interpoladores de grau um $(P\textsubscript{1}(x)$, interpolação linear), grau dois $(P\textsubscript{2}(x)$, interpolação quadrática) e grau três $(P\textsubscript{3}(x)$, interpolação cúbica) para determinar uma aproximação de $f(1,7)$ e encontre o erro absoluto para cada aproximação no ponto 1,7. Plote as funções $f(x)$, $P\textsubscript{1}(x)$,  $P\textsubscript{2}(x)$ e $P\textsubscript{3}(x)$ em um mesmo gráfico, inserindo \textit{nomes nos eixos}, \textit{título} e \textit{legenda}. Determine as equações dos polinômios $P\textsubscript{1}(x)$, $P\textsubscript{2}(x)$ e $P\textsubscript{3}(x)$. Faça uma interpretação dos resultados.
